...
每一个 `triggerix-*` 仓库的逐库调研。每个章节都遵循相同的结构：**身份 → 用途 → 目录布局 → 公共接口 → 依赖 → 值得注意的模式**。
...
> **仓库**：`triggerix-ai-component-native/`
...
> **仓库**：`triggerix-editor-preset-war3/`
...
> **仓库**：`triggerix-collective.github.io/`（私有）
